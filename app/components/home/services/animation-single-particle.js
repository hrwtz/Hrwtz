export default animationSingleParticle;
...
function animationSingleParticle () {
	...
	function getInstance (animationObject) {
		var serviceInstance = new Service(),
			computedStyle = getComputedStyle(animationObject.can);
		...
		serviceInstance.x = parseInt(computedStyle.width) / 2;
		serviceInstance.y = parseInt(computedStyle.height) / 2;
	...
	function update (){
		var computedStyle = getComputedStyle(this.animationObject.can);
		...
		if (
			this.opacity < 0.1 || 
			this.x + this.radius > parseInt(computedStyle.width) || 
			this.x - this.radius < 0 || 
			this.y + this.radius > parseInt(computedStyle.height) || 
