	...
	.factory('animationObjSplit', [function () {
        ...
        function Service (element) {
            ...
            this.init = function(triangleService){
                ...
                self.split[0].Coords[2] = getMidPoint(self.triangle.triCoords[0], self.triangle.triCoords[1], .3);
                self.split[0].Coords[3] = getMidPoint(self.triangle.triCoords[1], self.triangle.triCoords[2], .6);
                self.split[0].vx = -.5;
                self.split[0].vy = -1.3;
                self.split[0].vrotate = .002;
                self.split[0].rotate = 0;

                // Get split shape 1 coords
                self.split[1].Coords[0] = self.triangle.triCoords[1];
                self.split[1].Coords[1] = getMidPoint(self.triangle.triCoords[0], self.triangle.triCoords[1], .3);
                self.split[1].Coords[2] = getMidPoint(self.triangle.triCoords[1], self.triangle.triCoords[2], .6);
                self.split[1].vx = .5;
                self.split[1].vy = 1.3;
                self.split[1].vrotate = .0005;
