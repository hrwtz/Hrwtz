export default animationObjBackground;
...
function animationObjBackground ($timeout, $filter) {
	...
	function init () {
		this.color = $filter('rgb2hex')(getComputedStyle(this.element[0])['background-color']);
	...
	function draw () {
		...
		this.ctx.fillRect(0, 0, parseInt(computedStyle.width), parseInt(computedStyle.height));
		this.ctx.restore();
	...
	function update () {
		var i = 0,
			computedStyle = getComputedStyle(this.can);
		this.circles.forEach(function(el){
			// If radius is bigger than canvas, remove from array and change bg color
			if (el.radius > parseInt(computedStyle.width) && el.radius > parseInt(computedStyle.height)){
				this.color = el.fill;
